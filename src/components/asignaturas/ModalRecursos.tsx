...
interface ModalProps {
  ...
  setRecursosEditar: (obj: Objetivo | null) => void;
}

const ModalRecursos: React.FC<ModalProps> = ({
  setShowModal,
  setRecursos,
  recursosEditar,
  setRecursosEditar,
  ...
  const handleSave = async () => {
    try {
      ...
      if (recursosEditar?.id) {
        ...
        if (res.data.status) {
          setRecursos((prev) => prev.map((r) => (r.id === recursosEditar.id ? res.data.recurso : r)));
        ...
        const res = await axios.post("/api/asignaturas/recursos", { tipo, descripcion, asignaturaId: materiaId });
        if (res.data.status) {
          setRecursos((prev) => [...prev, res.data.recurso]);
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center p-4 sm:p-6 overflow-y-auto">
      ...
      >
        <div className="flex items-center justify-between p-6 border-b border-gray-100 dark:border-gray-800">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-black text-gray-900 dark:text-white">
              {recursosEditar ? "Editar Recurso" : "Nuevo Recurso"}
            </h2>
          </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <select
                ...
                className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-emerald-500 outline-none transition-all appearance-none cursor-pointer"
              >
                <option value="">Selecciona el formato</option>
                <option value="digital">Material Digital (Software, PDF, URL)</option>
                <option value="fisico">Material Físico (Libros, Equipos)</option>
              ...
              <div className="dark:text-gray-100 rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700">
                <JoditEditor
                  ...
                  config={{
                    ...
                    placeholder: "Lista los materiales o enlaces necesarios...",
                    colors: {
        ...
        <div className="p-6 bg-gray-50/50 dark:bg-gray-800/30 border-t border-gray-100 dark:border-gray-800 flex justify-end gap-3">
          ...
          <button onClick={handleSave} className="flex items-center gap-2 px-8 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl shadow-lg shadow-emerald-600/20 transition-all active:scale-95 font-bold text-sm" disabled={loading}>
            {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <span>{recursosEditar ? "Actualizar" : "Guardar"}</span>}
          </button>
        </div>
      </motion.div>
    </div>
